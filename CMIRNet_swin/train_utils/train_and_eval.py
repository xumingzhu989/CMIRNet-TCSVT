...
def evaluate(model, data_loader, bert_model, device, num_classes, args):
    model.eval()
    ...
    with torch.no_grad():
        for image, target, sentences, attentions in metric_logger.log_every(data_loader, 100, header):
            ...
            if args.small:
                ...
                metric_logger.update(loss=loss.item(), lr=0.1)
    ...
    if args.small:
        return confmat, iou,overallIOU, val_info, metric_logger.meters["loss"].global_avg
    else:
        return confmat, iou,overallIOU, val_info


def train_one_epoch(model, optimizer, data_loader, device, epoch, bert_model, lr_scheduler, val_loader, print_freq=10, scaler=None, args=None):
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    if args.small:
        overallIOU = (cum_I * 100. / cum_U)
        return metric_logger.meters["loss"].global_avg, lr, overallIOU
    else:
        return metric_logger.meters["loss"].global_avg, lr
