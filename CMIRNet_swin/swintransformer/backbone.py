...
from lib.mmcv_custom import load_checkpoint
from mmseg.utils import get_root_logger
from src.ASPP import ASPP_v4
...
class SwinTransformer_TGMM(nn.Module):
    def __init__(self,
        ...
        ###################################################################
        self.aspp = ASPP_v4(embed_dim * 8, embed_dim * 2, [3, 6, 9])
        self.TGMM_aspp = TGMM(embed_dim * 8,  # both the visual input and for combining, num of channels
                           embed_dim * 8,  # v_in
                           768,  # l_in
                           embed_dim * 8,  # key
                           embed_dim * 8,  # value
    ...
    def forward(self, x, l, l_mask,emb):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
             # x_cmae is (B, H*W, dim) 输入到CMAE, x is shape of (B, H/2*W/2, dim) 输入到后续的Block中, l_new 是新的句向量, word_new 是新的词向量
            x_cmae, H, W, x, Wh, Ww, att, l_new, word_new = layer(x, Wh, Ww, l, l_mask,emb)

            if i in self.out_indices:
                out = x_cmae.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                outs.append(out)
                atts.append(att)
                lans.append(l_new)
                words.append(word_new)
        #######################################################
        x = x.view(-1, H, W, self.num_features[3]).permute(0, 3, 1, 2).contiguous()   # x is BCHW
        xaspp = self.aspp(x)
        xaspp = xaspp.permute(0,2,3,1).view(-1, H*W, self.num_features[3])  # xaspp_TGMM is B,HW,C
        x_cmae_aspp, l_aspp, word_aspp = self.TGMM_aspp(xaspp, l, l_mask,emb)
        xaspp = xaspp + x_cmae_aspp  # B,HW,C

        outs.append(x_cmae_aspp.view(-1, H, W, self.num_features[3]).permute(0, 3, 1, 2).contiguous())
        # atts.append(att)
        lans.append(l_aspp)
        words.append(word_aspp)
        #######################################################
        
        return tuple(outs), tuple(atts), tuple(lans), tuple(words), xaspp.view(-1, H, W, self.num_features[3]).permute(0, 3, 1, 2).contiguous()  # 最后一项，重整维度，然后输入到CGIP中

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinTransformer_TGMM, self).train(mode)
        self._freeze_stages()


###############################################################
class BasicLayer_TGMM(nn.Module):
    def __init__(self,
                 dim,
                 depth,
                 num_heads,
        ...
        if self.block_i > 0:  # 只对1,2,3 block进行TGMM操作
            self.TGMM = TGMM(dim,  # both the visual input and for combining, num of channels
                                dim,  # v_in
                                768,  # l_in
                                dim,  # key
                                dim,  # value
    ...
    def forward(self, x, H, W, l, l_mask,emb):
        """ Forward function.
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                x = blk(x, attn_mask)  # x output of a Block has shape (B, H*W, dim)

        if self.block_i == 0:  # 只对1,2,3 block进行TGMM操作
            x_residual, l_new, word_new = x, None, None
        elif self.block_i > 0:
            x_residual, l_new, word_new = self.TGMM(x, l, l_mask,emb)  # x_residual is shape of (B, H*W, dim)
            x = x + x_residual
        att = [1]
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x_residual, H, W, x_down, Wh, Ww, att, l_new, word_new  # x_residual is (B, H*W, dim), x_down is shape of (B, H/2*W/2, dim), l_new 是新的句向量, word_new 是新的词向量
        else:
            return x_residual, H, W, x, H, W, att, l_new, word_new  # x_residual is (B, H*W, dim), x is shape of (B, H/2*W/2, dim), l_new 是新的句向量, word_new 是新的词向量
###############################################################
#############################################################################################################
class TGMM(nn.Module):
    ...
    def forward(self, x, l, l_mask,emb):
        ...
        return mm, l_new, word_new  # l_new是新的句向量，word是新的词向量
...
class SpatialImageLanguageAttention(nn.Module):
    def __init__(self, v_in_channels, l_in_channels, key_channels, value_channels, out_channels=None, num_heads=1):
        ...
        self.W2 = nn.Sequential(
            nn.Conv1d(self.value_channels, 512, kernel_size=1, stride=1),
        )
        self.W3 = nn.Sequential(
            nn.Conv1d(self.value_channels, 512, kernel_size=1, stride=1),
        )
        self.project_emb = nn.Sequential(
            nn.Conv1d(self.l_in_channels, 512, kernel_size=1, stride=1),
        )
    ...
    def forward(self, x, l, l_mask,emb):
        ...
        # value2 = value.reshape(B, self.num_heads, self.value_channels//self.num_heads, n_l)
        ...
        sim_map_ori = F.softmax(sim_map, dim=-1)  # (B, num_heads, h*w, N_l)

        sim_map = nn.AdaptiveAvgPool2d([1,n_l])(sim_map_ori) #(B,num_heads,1,N_l)
        
        emb = emb.unsqueeze(-1)
        emb = self.project_emb(emb)
        value = (sim_map * value) #(B,head,C,N_l)

        l_new = value.reshape(B, self.value_channels, n_l)
        word_new = self.W3(l_new)
        l_new = nn.AdaptiveAvgPool2d([self.value_channels, 1])(l_new)#(B,C,1)
        
        # print(l_new.shape)
        l_out = self.W2(l_new) + emb

        out = torch.matmul(sim_map_ori, value.permute(0,1,3,2)).permute(0,1,3,2).contiguous().reshape(B,self.value_channels,HW)
        out = self.W1(out)  # (B, value_channels, HW)

        return out,l_out,word_new