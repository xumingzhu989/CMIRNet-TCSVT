...
class CGIP_knn(nn.Module):
    def __init__(self, dim=256, num_heads=8):
        ...
        self.conv_V = nn.Sequential(
            nn.Conv2d(2048, dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(dim),
            nn.ReLU()
        )
        # self.conv_V = nn.Sequential(
        ...
        # self.CGI = DyGraphConv2d(dim, dim, kernel_size=9, dilation=1)
    ...
    def forward(self, Vx, Tx):
        ...
        # out = self.CGI(Vx, Tx)
        l, l3, l4, l5, laspp = Tx
        L = self.conv_T(l) + l3 + l4 + l5 + laspp
        out = self.CGI(Vx, L)
        out = self.IGR(out)
        return out + Vx
...
class MH_DyGraphConv2d(nn.Module):
    def __init__(self, dim, num_heads=8):
        ...
        self.graphconv = nn.ModuleList(
            [DyGraphConv2d(dim // num_heads, dim // num_heads, kernel_size=9, dilation=1, conv='edge', act='gelu') for i
             in range(num_heads)])
...
class IGR_MH(nn.Module):
    ...
    def forward(self, Vx):
        ...
        query, key, value = self.qkv(Vx).reshape(B, self.num_heads, C // self.num_heads * 3, N_node).chunk(3,
                                                                                                           dim=2)  # B,head,C_head,N_node
...
def xy_dense_knn_matrix(x, y, k=16):
    ...
    with torch.no_grad():
        ...
        dist = xy_pairwise_distance(x.detach(), y.detach()) # B, HW, T
        _, nn_idx = torch.topk(-dist, k=k)
        center_idx = torch.arange(0, n_points, device=x.device).repeat(batch_size, k, 1).transpose(2, 1)
    return torch.stack((nn_idx, center_idx), dim=0) # 2, B, HW, k
...
class EdgeConv2d(nn.Module):
    ...
    def forward(self, x, edge_index, y=None):  # x is B,C,N,1
        x_i = batched_index_select(x, edge_index[
            1])  # edge_index[1]保存的是行号，x_i是按行提取的特征（B,C,N,K），对于N中的第i行的第i个节点，其有K列，即第i个节点特征有K份；
        x_j = batched_index_select(y, edge_index[
            0])  # edge_index[0]保存的是列号，x_j是按列提取的特征（B,C,N,K），对于N中的第j行的第j个节点，其有K列，即第i个节点的K个邻居节点的特征；
