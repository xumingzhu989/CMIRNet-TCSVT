...
class CMAE_512(nn.Module):
    def __init__(self,dim=256):
        ...
        self.ConvBNRelu5_1 = nn.Sequential(
            nn.Conv2d(dim + dim, dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(dim),
            nn.ReLU(),
            nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=False),
        ...
        self.ConvBNRelu5_2 = nn.Sequential(
            nn.Conv2d(dim + dim, dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(dim),
            nn.ReLU(),
            nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=False),
        ...
        self.ConvBNRelu2 = nn.Sequential(
            nn.Conv2d(dim + 256, dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(dim),
            nn.ReLU(),
            nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(dim),
        ...
        self.ConvBNRelu1 = nn.Sequential(
            nn.Conv2d(dim + 64, dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(dim),
            nn.ReLU(),
            nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(dim),
        ...
        self.fusion5 = Fusion(dim=dim)
        self.fusionaspp = Fusion(dim=dim)

        self.up1 = nn.Upsample(scale_factor=1, mode='bilinear', align_corners=True)
        self.up2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)

    def forward(self, res, l, mask, lans, emb):
        # x(B,Cx,H,W), Cx = 64, 256, 512, 1024, 2048, 256
        # l(B,Cl,N)
        # emb(B,Cl)
        # mask(B,N,1)
        x1, x2, x3, x4, x5, x_aspp, x_gcn = res
        l3, l4, l5, l_aspp = lans

        x3 = self.linear_x_3(x3)
        x4 = self.linear_x_4(x4)
        x5 = self.linear_x_5(x5)
        ...
        res5 = self.ConvBNRelu5_1(torch.cat((res_aspp, x5), dim=1))
        cmae5 = self.fusion5(res5, l5)
        res5 = self.ConvBNRelu5_2(torch.cat((res5, cmae5), dim=1))

        res4 = self.ConvBNRelu4_1(torch.cat((self.up2(res5), x4), dim=1))
        ...
        res2 = self.ConvBNRelu2(torch.cat((self.up2(res3), x2), dim=1))

        res1 = self.ConvBNRelu1(torch.cat((self.up2(res2), x1), dim=1))

        return tuple([res1, res2, res3, res4, res5, res_aspp])
